...
##################
# favicon and title of the page in the browser tab
st.set_page_config(
            page_title="Kitchen AI",
            page_icon=":cooking:",
            # layout="centered",
            # initial_sidebar_state="auto"
            )

def get_recipes_expand(response):
    """ Returning top 5 recipes each in an st.expander """
    index_list = list(response['suggest_recipes'].keys())

    for i, index in enumerate(index_list):
        column_names = response['suggest_recipes'][index]
        ...
        # with st.expander(f"{column_names[index_list[0]]['Name']}"):
...
st.subheader(f"I can see ..[include ingredients shere]")
...
st.button("Let's get some recipes!")

# get_receipes from response
st.markdown(f"### These are the top 5 recipes of closest match to the ingredients and preferences provided:")
get_recipes_expand(response)
